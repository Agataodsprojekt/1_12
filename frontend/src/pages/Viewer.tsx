...
// Komponent małego toolbara z przyciskami Home i Fit
const CameraControlsToolbar = ({ onHome, onFit, theme }: { onHome: () => void; onFit: () => void; theme: string }) => {
  const [homeTooltipVisible, setHomeTooltipVisible] = useState(false);
  const [fitTooltipVisible, setFitTooltipVisible] = useState(false);
    ...
    <div style={{
      ...
      <div style={{ position: 'relative' }}>
        <button
          ...
          style={{
            ...
            border: `1px solid ${theme === 'dark' ? 'rgba(75, 85, 99, 0.5)' : 'rgba(229, 231, 235, 0.5)'}`,
            borderRadius: '8px',
            cursor: 'pointer',
            transition: 'all 0.2s',
            boxShadow: '0 2px 8px rgba(0, 0, 0, 0.15)',
          }}
          onMouseEnter={() => setHomeTooltipVisible(true)}
          onMouseLeave={() => setHomeTooltipVisible(false)}
        >
          <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ color: theme === 'dark' ? '#f9fafb' : '#111827' }}>
      ...
      <div style={{ position: 'relative' }}>
        <button
          ...
          onMouseEnter={() => setFitTooltipVisible(true)}
          onMouseLeave={() => setFitTooltipVisible(false)}
        >
          <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ color: theme === 'dark' ? '#f9fafb' : '#111827' }}>
...
const Viewer = () => {
  ...
  const thatOpenGridRef = useRef<any>(null); // Referencja do gridu ThatOpen
  const initialCameraStateRef = useRef<CameraState | null>(null); // Początkowy stan kamery dla przycisku Home
  ...
  // Stan dla Documentation Panel
  const [showDocumentationPanel, setShowDocumentationPanel] = useState(false);
  const showDocumentationPanelRef = useRef(setShowDocumentationPanel);
  
  // Synchronizuj ref z funkcją setState
  useEffect(() => {
    showDocumentationPanelRef.current = setShowDocumentationPanel;
  }, []);
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      // Zapisz do historii akcji
      saveCameraState();
      
      // Zapisz również do ref dla przycisku Home
      try {
        const camera = viewerRef.current?.camera as OBC.OrthoPerspectiveCamera;
        if (camera && camera.controls && camera.get) {
          ...
          if (threeCamera && threeCamera.position) {
            ...
            if (target) {
              ...
              console.log("📷 Initial camera state saved to ref for Home button");
            }
          }
        }
      } catch (error) {
        console.warn('⚠️ Could not save initial camera state to ref:', error);
      }
    }, 1000);
    ...
    // --- GRID (THATOPEN STYLE) ---
    // Sprawdź dostępność OBC.Grids i utwórz grid w stylu ThatOpen
    try {
      // Sprawdź czy OBC.Grids jest dostępny
      const GridsClass = (OBC as any).Grids;
      if (GridsClass) {
        // Utworz obiekt world zgodny z interfejsem ThatOpen
        const world = {
        ...
        // Pobierz Grids manager - spróbuj różne metody
        let grids = null;
        try {
          grids = viewer.get(GridsClass);
        } catch (e) {
          // Fallback: spróbuj utworzyć instancję bezpośrednio
          try {
            grids = new GridsClass(viewer);
          } catch (e2) {
            console.warn('⚠️ Could not create Grids instance:', e2);
          }
        }
        
        if (grids && typeof grids.create === 'function') {
          thatOpenGridRef.current = grids.create(world);
          console.log('✅ ThatOpen Grid created successfully');
        } else {
          console.warn('⚠️ OBC.Grids.create() not available, using enhanced fallback grid');
          // Fallback: utworz ulepszony grid z fade-out
          thatOpenGridRef.current = createEnhancedGrid(scene, theme);
        }
      } else {
        console.warn('⚠️ OBC.Grids not available in this version, using enhanced fallback grid');
        // Fallback: utworz ulepszony grid z fade-out
        thatOpenGridRef.current = createEnhancedGrid(scene, theme);
      }
    } catch (error) {
      console.warn('⚠️ Error initializing ThatOpen Grid, using enhanced fallback:', error);
      // Fallback: utworz ulepszony grid z fade-out
      thatOpenGridRef.current = createEnhancedGrid(scene, theme);
    }
    
    // Funkcja pomocnicza do tworzenia ulepszonego gridu z fade-out
    function createEnhancedGrid(scene: THREE.Scene, currentTheme: string) {
      const gridSize = 10000;
      const gridDivisions = 5000;
      const backgroundColor = scene.background instanceof THREE.Color 
        ? scene.background 
        : (currentTheme === "dark" ? new THREE.Color(0x202932) : new THREE.Color(0xE6E7E4));
      
      const isDarkMode = currentTheme === "dark";
      const baseGridColor = isDarkMode ? 0x4a4a4a : 0x999999;
      ...
      for (let i = 0; i < numLayers; i++) {
        ...
        const layerSize = gridSize * (0.2 + layerProgress * 0.8);
        const layerDivisions = Math.floor(gridDivisions * (0.2 + layerProgress * 0.8));
        
        const fadeStart = 0.1;
        const adjustedProgress = Math.max(0, (layerProgress - fadeStart) / (1 - fadeStart));
        const opacityEase = 1.0 - Math.pow(adjustedProgress, 1.2);
        const layerOpacity = baseOpacity * opacityEase;
        
        if (layerOpacity < 0.01) continue;
        
        const gridColor = new THREE.Color(baseGridColor);
        const fadedColor = new THREE.Color(backgroundColor);
        fadedColor.lerp(gridColor, opacityEase);
        ...
        if (layerGrid.material instanceof THREE.LineBasicMaterial) {
          layerGrid.material.opacity = layerOpacity;
          layerGrid.material.transparent = layerOpacity < 1.0;
          layerGrid.material.color = fadedColor;
        }
    ...
    // --- OSIE WSPÓŁRZĘDNYCH ---
    // Zwiększona długość osi proporcjonalnie do większej siatki
    const axesLength = 500; // Długość osi: 500 jednostek (proporcjonalnie do siatki 10000)
    ...
    ifcLoader.onIfcLoaded.add(async (model: any) => {
      ...
      highlighter.events.select.onHighlight.add(async (selection: any) => {
        ...
        // Use longer delay and retry mechanism to ensure Properties panel is ready
        setTimeout(() => {
    ...
    // Dodajemy przycisk testowy bezpośrednio do DOM toolbara po jego utworzeniu
    // To podejście jest bardziej niezawodne niż próba użycia addChild z własnym elementem
    const addTestButton = () => {
      try {
        // Funkcja pomocnicza do znajdowania toolbara
        const findToolbar = (): HTMLElement | null => {
          // Próba 1: Bezpośredni dostęp do domElement toolbara
          if ((mainToolbar as any).domElement) {
            return (mainToolbar as any).domElement;
          }
          
          // Próba 2: Szukanie w DOM po klasach
          const selectors = [
            '.tooeen-toolbar',
            '[class*="toolbar"]',
            '[data-tooeen-name="toolbar"]'
          ];
          ...
          // Próba 3: Szukanie elementu który zawiera przyciski Load IFC i Properties
          const allElements = document.querySelectorAll('button, [class*="ui"], [class*="component"]');
          for (const el of Array.from(allElements)) {
            const text = el.textContent || '';
            if (text.includes('Load') || text.includes('IFC') || el.getAttribute('title')?.includes('IFC')) {
              const parent = el.closest('[class*="toolbar"], [class*="ui"]');
        ...
        const toolbarElement = findToolbar();
        
        if (toolbarElement) {
          // Sprawdzamy czy przycisk już istnieje - jeśli tak, aktualizujemy go zamiast tworzyć nowy
          let testButtonElement = toolbarElement.querySelector('[data-tooeen-name="documentation-button"]') as HTMLButtonElement || 
                                  toolbarElement.querySelector('[data-tooeen-name="test-button"]') as HTMLButtonElement;
          
          const isExistingButton = !!testButtonElement;
          
          if (testButtonElement) {
            // Przycisk już istnieje - aktualizujemy tylko zawartość
            console.log("🔄 Aktualizuję istniejący przycisk dokumentacji");
          } else {
            // Przycisk nie istnieje - tworzymy nowy
            testButtonElement = document.createElement("button");
            console.log("➕ Tworzę nowy przycisk dokumentacji");
          }
          
          // Tworzymy/przygotowujemy przycisk dokumentacji (zastępuje przycisk "View Project documentation")
          // Używamy tego samego stylu co przyciski OpenBIM Components (Load IFC i Properties)
          
          // Sprawdzamy style istniejących przycisków w toolbara, aby dopasować nasz przycisk
          const existingButtons = toolbarElement.querySelectorAll('button, [class*="button"], [class*="ui"]');
          let referenceButton: HTMLElement | null = null;
          
          // Szukamy pierwszego przycisku który wygląda jak przycisk OpenBIM Components
          for (const btn of Array.from(existingButtons)) {
            const btnElement = btn as HTMLElement;
            // Sprawdzamy czy to przycisk z toolbara (nie z innych miejsc)
            if (btnElement.closest('.tooeen-toolbar') || btnElement.getAttribute('data-tooeen-name')) {
              referenceButton = btnElement;
              break;
            }
          }
          
          // Jeśli nie znaleźliśmy, szukamy w strukturze toolbara
          if (!referenceButton && toolbarElement.children.length > 0) {
            const firstChild = toolbarElement.children[0] as HTMLElement;
            const buttonInChild = firstChild.querySelector('button, [class*="button"]');
            if (buttonInChild) {
              referenceButton = buttonInChild as HTMLElement;
            }
          }
          
          if (!isExistingButton) {
            // Tylko dla nowego przycisku - kopiujemy klasy CSS i style
            if (referenceButton) {
              // Kopiujemy wszystkie klasy CSS
              testButtonElement.className = referenceButton.className;
              // Resetujemy style inline, aby używać tylko klas CSS
              testButtonElement.style.cssText = '';
              // Kopiujemy tylko podstawowe właściwości, które mogą być potrzebne
              const computedStyle = window.getComputedStyle(referenceButton);
              testButtonElement.style.display = computedStyle.display || 'inline-flex';
              testButtonElement.style.alignItems = computedStyle.alignItems || 'center';
              testButtonElement.style.justifyContent = computedStyle.justifyContent || 'center';
            } else {
              // Fallback - używamy typowych klas OpenBIM Components
              testButtonElement.className = "tooeen-button tooeen-simple-ui-component";
              testButtonElement.style.cssText = '';
            }
            
            testButtonElement.setAttribute("data-tooeen-name", "documentation-button");
            
            // Kopiujemy również inne atrybuty, jeśli istnieją (tylko dla nowego przycisku)
            if (referenceButton) {
              const dataAttributes = Array.from(referenceButton.attributes).filter(attr => 
                attr.name.startsWith('data-') || attr.name === 'role' || attr.name === 'aria-label'
              );
              dataAttributes.forEach(attr => {
                if (attr.name !== 'data-tooeen-name') { // Nie nadpisujemy naszego data-tooeen-name
                  testButtonElement.setAttribute(attr.name, attr.value);
                }
              });
            }
            
            // Ustawiamy obsługę kliknięcia (tylko dla nowego przycisku)
            testButtonElement.onclick = (e) => {
              e.preventDefault();
              e.stopPropagation();
              // Otwieramy panel dokumentacji (zastępuje funkcjonalność przycisku "View Project documentation")
              if (showDocumentationPanelRef.current) {
                showDocumentationPanelRef.current(true);
                console.log("📄 Documentation panel opened from toolbar");
              }
            };
            
            // Dodajemy przycisk do toolbara (tylko dla nowego przycisku)
            toolbarElement.appendChild(testButtonElement);
          }
          
          // Aktualizujemy zawartość przycisku (zarówno dla nowego jak i istniejącego)
          // Sprawdzamy strukturę istniejących przycisków w toolbara
          if (!referenceButton) {
            const existingButtons = toolbarElement.querySelectorAll('button, [class*="button"], [class*="ui"]');
            for (const btn of Array.from(existingButtons)) {
              const btnElement = btn as HTMLElement;
              if (btnElement !== testButtonElement && (btnElement.closest('.tooeen-toolbar') || btnElement.getAttribute('data-tooeen-name'))) {
                referenceButton = btnElement;
                break;
              }
            }
          ...
          // Aktualizujemy zawartość przycisku - tylko emotka 📄
          if (referenceButton && referenceButton.innerHTML) {
            // Jeśli istniejący przycisk ma innerHTML, używamy podobnej struktury
            testButtonElement.innerHTML = `📄`;
          } else {
            // Prostsza struktura
            testButtonElement.textContent = "📄";
          }
          
          // Aktualizujemy tooltip
          testButtonElement.title = "View project documentation";
          
          console.log(`✅ Przycisk dokumentacji ${isExistingButton ? 'zaktualizowany' : 'dodany'} do toolbara`);
        } else {
          console.warn("⚠️ Nie znaleziono toolbara, próbuję ponownie...");
          // Próbujemy ponownie po dłuższym czasie
          setTimeout(addTestButton, 500);
        }
      } catch (error) {
        console.error("❌ Błąd podczas dodawania przycisku testowego:", error);
      }
    };
    
    // Próbujemy dodać przycisk po krótkim opóźnieniu
    setTimeout(addTestButton, 200);
    
    // Add Documentation button to Properties panel header using DOM manipulation
    // This approach is safer and doesn't interfere with OpenBIM toolbar structure
    const addDocumentationButtonToProperties = () => {
      try {
        // Find Properties panel - try multiple selectors
        const selectors = [
        ...
        if (!propertiesPanel) {
          const allPanels = document.querySelectorAll('[class*="panel"], [class*="Panel"], [class*="ui"]');
          for (const panel of Array.from(allPanels)) {
            const text = panel.textContent || '';
            if (text.includes('Properties') || text.includes('properties') || text.includes('Element Properties')) {
        ...
        if (!propertiesPanel) {
          console.warn('⚠️ Properties panel not found, retrying...');
          return false;
        }
        
        // Check if button already exists
        if (propertiesPanel.querySelector('.documentation-button')) {
          return true; // Button already exists
        }
        
        // Find the header/title area of Properties panel
        const header = propertiesPanel.querySelector('header, .header, [class*="header"], [class*="title"], .title') || propertiesPanel.firstElementChild;
        
        if (!header) {
          console.warn('⚠️ Properties panel header not found');
          return false;
        }
        ...
        docButton.innerHTML = `
          <span class="material-icons-outlined" style="font-size: 20px; margin-right: 4px;">description</span>
          <span style="font-size: 14px;">Documentation</span>
        `;
        docButton.title = "View project documentation";
        ...
        docButton.addEventListener("mouseenter", () => {
          docButton.style.background = "rgba(255, 255, 255, 0.2)";
        });
        docButton.addEventListener("mouseleave", () => {
          docButton.style.background = "rgba(255, 255, 255, 0.1)";
        });
        docButton.addEventListener("click", (e) => {
          e.stopPropagation();
          if (showDocumentationPanelRef.current) {
            showDocumentationPanelRef.current(true);
            console.log("📄 Documentation panel opened from Properties panel");
          }
        });
        
        // Add button to header (append or insert)
        if (header instanceof HTMLElement) {
          // Try to find a container for buttons or add directly
          const buttonContainer = header.querySelector('.button-container, .actions, [class*="button"]');
          if (buttonContainer) {
            buttonContainer.appendChild(docButton);
          } else {
            // Add to header, try to place it at the end
            header.appendChild(docButton);
          }
        }
        
        console.log('✅ Documentation button added to Properties panel');
        return true;
      } catch (error) {
        console.error('❌ Error adding documentation button to Properties panel:', error);
        return false;
      }
    };
    
    // Try to add button immediately and also after a delay (when Properties panel is rendered)
    setTimeout(() => {
      if (!addDocumentationButtonToProperties()) {
        // Retry after Properties panel is fully loaded
        setTimeout(addDocumentationButtonToProperties, 1000);
      }
    }, 500);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      const backgroundColor = theme === "dark" 
        ? new THREE.Color(0x202932) // Ciemny granatowy
        : new THREE.Color(0xE6E7E4); // Jasny szary
      ...
      // Aktualizuj kolory siatki w zależności od motywu
      if (thatOpenGridRef.current) {
        try {
          // Sprawdź czy to ThatOpen Grid (ma metodę setColor) czy fallback grid (THREE.Group)
          if (typeof thatOpenGridRef.current.setColor === 'function') {
            // ThatOpen Grid - użyj metody setColor
            const gridColor = theme === "dark" ? 0x4a4a4a : 0x999999;
            thatOpenGridRef.current.setColor(new THREE.Color(gridColor));
          } else if (thatOpenGridRef.current instanceof THREE.Group) {
            // Fallback grid (THREE.Group) - zaktualizuj wszystkie warstwy
            const isDarkMode = theme === "dark";
            const baseGridColor = isDarkMode ? 0x4a4a4a : 0x999999;
            const baseOpacity = 0.35;
            
            thatOpenGridRef.current.children.forEach((layerGrid: THREE.Object3D, index: number) => {
              if (!(layerGrid instanceof THREE.GridHelper) || !layerGrid.material) return;
              
              const numLayers = thatOpenGridRef.current.children.length;
              const layerProgress = index / (numLayers - 1);
              const fadeStart = 0.1;
              const adjustedProgress = Math.max(0, (layerProgress - fadeStart) / (1 - fadeStart));
              const opacityEase = 1.0 - Math.pow(adjustedProgress, 1.2);
              const layerOpacity = baseOpacity * opacityEase;
              ...
              if (layerGrid.material instanceof THREE.LineBasicMaterial) {
                ...
                layerGrid.material.opacity = layerOpacity;
                layerGrid.material.transparent = layerOpacity < 1.0;
                layerGrid.material.needsUpdate = true;
              }
            });
          } else if (thatOpenGridRef.current.material) {
            // Pojedynczy grid helper - bezpośrednia aktualizacja materiału
            const gridColor = theme === "dark" ? 0x4a4a4a : 0x999999;
            if (thatOpenGridRef.current.material instanceof THREE.Material) {
              (thatOpenGridRef.current.material as any).color = new THREE.Color(gridColor);
              thatOpenGridRef.current.material.needsUpdate = true;
            }
          }
        } catch (error) {
          console.warn('⚠️ Could not update Grid colors:', error);
        }
      }
    }, 50);
  ...
  // Home - przywróć początkowy stan kamery
  const handleHome = () => {
    if (!viewerRef.current || !initialCameraStateRef.current) {
      console.warn('⚠️ Cannot restore home view - initial camera state not available');
      return;
    }
    ...
    try {
      ...
      console.log('🏠 Restoring camera to home position:', {
        position: state.position,
        target: state.target,
      });

      // Użyj setLookAt z animacją dla płynnego przejścia
      ...
      console.log('✅ Camera restored to home position');
    } catch (error) {
      console.error('❌ Error restoring home view:', error);
    }
  };

  // Fit View - dopasuj widok do wszystkich załadowanych modeli
  const handleFitView = () => {
    if (!viewerRef.current) {
      console.warn('⚠️ Viewer not available');
      return;
    }
    ...
    try {
      ...
      // Pobierz wszystkie załadowane modele
      const loadedModels = getLoadedModels();
      
      if (loadedModels.length === 0) {
        console.warn('⚠️ No models loaded, restoring home view instead');
        // Fallback: użyj home view jeśli nie ma modeli
      ...
      // Oblicz bounding box wszystkich modeli
      ...
      if (box.isEmpty()) {
        console.warn('⚠️ Could not calculate bounding box, restoring home view');
        if (initialCameraStateRef.current) {
          handleHome();
        }
        return;
      ...
      // Oblicz środek i rozmiar modelu
      const center = box.getCenter(new THREE.Vector3());
      const size = box.getSize(new THREE.Vector3());
      
      // Oblicz pozycję kamery - podobnie jak w ViewFactory.calculateStoreyCamera
      // Użyj największego wymiaru do obliczenia odległości
      const maxSize = Math.max(size.x, size.y, size.z);
      const distance = maxSize * 1.5; // Odległość 1.5x największego wymiaru
      
      // Pozycja kamery - przesunięta w górę i do tyłu
      ...
      // Target to środek modelu
      const target = center.clone();

      console.log('📐 Fitting view to models:', {
        center,
        size,
        cameraPosition,
        target,
      ...
      console.error('❌ Error fitting view to models:', error);
      // Fallback: spróbuj przywrócić home view
      if (initialCameraStateRef.current) {
        handleHome();
      }
    }
  ...
  const handleActionSelect = (action: string) => {
    ...
    // Obsługa Documentation (panel dokumentacji projektu)
    if (action === "documentation") {
      setShowDocumentationPanel(true);
      console.log("📄 Documentation panel enabled");
      return;
    }
    
    // Wyłącz panel dokumentacji gdy wybrana jest inna akcja
    if (showDocumentationPanel && action !== "documentation") {
      setShowDocumentationPanel(false);
      console.log("📄 Documentation panel disabled");
    }
  ...
  const handleAddComment = (text: string, elementId?: string, elementName?: string) => {
    ...
    if (elementId) {
      // Wait a bit longer to ensure comment is added to state
      setTimeout(() => {
  ...
  const addCommentsToPropertiesPanel = (elementId: string, retryCount: number = 0) => {
    console.log(`💬 [Comments] Attempting to add comments to Properties panel for element ${elementId} (attempt ${retryCount + 1})`);
    
    if (!viewerRef.current) {
      console.warn('⚠️ [Comments] Viewer not ready, retrying...');
      if (retryCount < 5) {
        setTimeout(() => {
          addCommentsToPropertiesPanel(elementId, retryCount + 1);
        }, (retryCount + 1) * 200);
      }
      return;
    }
    
    // Method 1: Try to get Properties panel through OpenBIM Components API
    let propertiesPanelElement: HTMLElement | null = null;
    
    if (viewerRef.current) {
      try {
        const propertiesPanelUI = viewerRef.current.ui?.projects?.active?.uiElements?.get("Properties");
        if (propertiesPanelUI) {
          // Try to get the DOM element from the UI component
          if (propertiesPanelUI.domElement) {
            propertiesPanelElement = propertiesPanelUI.domElement as HTMLElement;
            console.log('✅ Found Properties panel via OpenBIM API (domElement)');
          } else if ((propertiesPanelUI as any).get) {
            // Some UI components have a get() method
            const element = (propertiesPanelUI as any).get();
            if (element && element instanceof HTMLElement) {
              propertiesPanelElement = element;
              console.log('✅ Found Properties panel via OpenBIM API (get method)');
            }
          } else if ((propertiesPanelUI as any).innerElement) {
            propertiesPanelElement = (propertiesPanelUI as any).innerElement as HTMLElement;
            console.log('✅ Found Properties panel via OpenBIM API (innerElement)');
          }
          
          // If we have the UI component but not the element, try to find it in DOM
          if (!propertiesPanelElement) {
            console.log('⚠️ Found Properties UI component but not DOM element, searching in DOM...');
          }
        } else {
          console.log('⚠️ Properties panel UI component not found via OpenBIM API');
        }
      } catch (error) {
        console.warn('⚠️ Error accessing Properties panel via OpenBIM API:', error);
      }
    }
    
    // Method 2: Fallback to DOM selectors if API method didn't work
    if (!propertiesPanelElement) {
      const selectors = [
        '[data-tooeen-name="properties"]',
        '.properties-panel',
      ...
      for (const selector of selectors) {
        ...
        if (found) {
          propertiesPanelElement = found;
          console.log(`✅ Found Properties panel via DOM selector: ${selector}`);
          break;
        }
      }
    }
    
    // Method 3: Search by text content as last resort
    if (!propertiesPanelElement) {
      ...
      if (possiblePanel) {
        propertiesPanelElement = possiblePanel;
        console.log('✅ Found Properties panel via text content search');
      }
    }
    
    // If still not found, retry with delay
    if (!propertiesPanelElement) {
      if (retryCount < 5) {
        console.log(`⚠️ Properties panel not found, retrying in ${(retryCount + 1) * 200}ms...`);
        setTimeout(() => {
          addCommentsToPropertiesPanel(elementId, retryCount + 1);
        }, (retryCount + 1) * 200);
        return;
      } else {
        console.error('❌ Could not find Properties panel after 5 attempts');
        return;
      }
    }
    
    console.log('✅ Properties panel found, proceeding to add comments section');

    // Find the content container inside Properties panel - this is where we should add comments
    // OpenBIM Properties panel usually has a scrollable content area
    let contentContainer: HTMLElement | null = null;
    
    // Try to find the content/scrollable area
    const contentSelectors = [
      '.tooeen-properties-content',
      '[class*="content"]',
      '[class*="scroll"]',
      '[class*="body"]',
      'div[style*="overflow"]'
    ...
    for (const selector of contentSelectors) {
      const found = propertiesPanelElement.querySelector(selector) as HTMLElement;
      if (found) {
        contentContainer = found;
        console.log(`✅ Found content container via selector: ${selector}`);
        break;
      }
    }
    
    // If no specific content container found, use the panel itself or its last child
    if (!contentContainer) {
      // Try to find the last scrollable div or the main content area
      const children = Array.from(propertiesPanelElement.children) as HTMLElement[];
      ...
      if (scrollableChild) {
        contentContainer = scrollableChild;
        console.log('✅ Found scrollable content container');
      } else if (children.length > 0) {
        // Use the last child or the panel itself
        contentContainer = children[children.length - 1];
        console.log('✅ Using last child as content container');
      } else {
        contentContainer = propertiesPanelElement;
        console.log('✅ Using Properties panel itself as content container');
      }
    }
    
    // Usuń poprzednią sekcję komentarzy jeśli istnieje
    const existingCommentsSection = contentContainer.querySelector('.custom-comments-section');
    if (existingCommentsSection) {
      existingCommentsSection.remove();
      console.log('🗑️ Removed existing comments section');
    }
    ...
    console.log(`📝 Found ${elementComments.length} comment(s) for element ${elementId}`);
    ...
    `;
      ...
      const shouldExpandByDefault = elementComments.length > 0; // Rozwiń jeśli są komentarze
      commentsContentContainer.style.cssText = `
        margin-top: 12px;
        display: ${shouldExpandByDefault ? 'block' : 'none'};
      `;
      
      // Funkcja rozwijania/zwijania
      let isExpanded = shouldExpandByDefault;
      if (shouldExpandByDefault) {
        arrowIcon.style.transform = 'rotate(180deg)';
      }
      header.addEventListener('click', () => {
        isExpanded = !isExpanded;
        commentsContentContainer.style.display = isExpanded ? 'block' : 'none';
      ...
      if (elementComments.length > 0) {
        elementComments.forEach((comment: Comment) => {
          ...
          if (deleteBtn) {
            deleteBtn.addEventListener('click', (e) => {
              ...
              // Odśwież sekcję
              setTimeout(() => addCommentsToPropertiesPanel(elementId), 300);
            });
          }

          commentsContentContainer.appendChild(commentDiv);
        });
      } else {
        ...
        `;
        commentsContentContainer.appendChild(emptyState);
      }
      ...
      commentsContentContainer.appendChild(hint);
      
      // Dodaj kontener z zawartością do sekcji komentarzy
      commentsSection.appendChild(commentsContentContainer);

      // Dodaj sekcję do panelu Properties - na końcu content container
      contentContainer.appendChild(commentsSection);
      console.log('✅ Comments section added to Properties panel');
  ...
  return (
    <div style={{ width: '100%', height: '100vh', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      >
        ...
        {/* Camera Controls Toolbar - mały toolbar po prawej stronie */}
      ...
      {/* Panel dokumentacji projektu */}
      {showDocumentationPanel && (
        <DocumentationPanel
          elements={elements}
          loadedModels={getLoadedModels()}
          onClose={() => setShowDocumentationPanel(false)}
        />
      )}
